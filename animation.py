...
class AnimationConfig(BaseModel):
    ...
    agents_num : typing.Optional[int] = 1
...
class AnimationMonitor(gym.Wrapper):
    ...
    def reset_animation(self, **kwargs):
        self._episode_idx += 1
        self.dones_history = [[False for _ in range(self.grid_config.num_agents)]]
        self.agents_xy_history = [deepcopy(self.grid.positions_xy)]
        self.targets_xy_history = [deepcopy(self.grid.finishes_xy)]
        ...
        if anim_cfg.show_agents:
            ...
            for agent_idx in range(anim_cfg.agents_num):
                ...
                if not anim_cfg.static:
                    ...
                    self.animate_field_of_view(field_of_view, agent_idx, gh)
    ...
    def animate_obstacles(self, obstacles, grid_holder, animation_config):
        ...
        :param grid_holder: 包含环境信息的 GridHolder 对象
        ...
        for i in range(gh.height):
            for j in range(gh.width):
                ...
                for step_idx, agents_xy in enumerate(gh.agents_xy_history[:gh.episode_length]):
                    ...
                    # 遍历每个智能体
                    for agent_idx, (ego_x, ego_y) in enumerate(agents_xy):
                        # 如果障碍物在智能体的可见范围内
                        if self.check_in_radius(x, y, ego_x, ego_y, self.grid_config.obs_radius):
                            seen.add((x, y))
                            break  # 一旦有智能体看到障碍物，跳出内部循环
...
def main():
    grid_config = GridConfig(size=8, num_agents=3, obs_radius=2, seed=12)
    ...
    print([env.action_space.sample() for _ in range(grid_config.num_agents)])
    env.save_animation("out.svg")
    env.save_animation("out-ego.svg", AnimationConfig(egocentric_idx=0,agents_num = env.grid_config.num_agents))
    env.save_animation('asd.svg', AnimationConfig(egocentric_idx=0,agents_num = env.grid_config.num_agents,show_agents=False))
